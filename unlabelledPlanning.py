...
nAgents = 12 # Number of agents at training time
...
nTrain = 55000 # Number of training samples
nValid = 125 # Number of valid samples
nTest = 0 # Number of testing samples
...
initMinDist = 1.5 # No two agents are located at a distance less than this
...
if doLocalGNN:
    ...
    hParamsLocalGNN['dimNodeSignals'] = [2*(3 * degree + 2)+1, 64] # Features per layer
...
for realization in range(nRealizations):
    ...
    plt.plot(np.arange(0, len(thisTrainVars['lossValid']) * validationInterval, validationInterval), thisTrainVars['lossValid'])
    plt.show()